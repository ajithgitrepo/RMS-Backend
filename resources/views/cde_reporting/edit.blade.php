    ...
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          ...
          <form method="post" action="{{ route('cde_reporting.update', $cde_reporting_to[0]->id ) }}" autocomplete="off" class="form-horizontal">
            @csrf
            @method('put')
            ...
            <div class="card ">
              <div class="card-header card-header-rose card-header-icon">
                ...
                <h4 class="card-title">{{ __('Edit CDE Reporting') }}</h4>
              </div>
              ...
              <div class="card-body ">
                     ...
                     value="{{ old('leave_type') }}" aria-required="true">
                       <option value=" ">Select</option>
                                 @foreach($merchant as $emp)
                                    <option value="{{ $emp->employee_id }}" {{$cde_reporting_to[0]->merchandiser_id == $emp->employee_id  ? 'selected' : ''}}  >{{ $emp->first_name }} {{ $emp->middle_name }} {{ $emp->surname }} ({{ $emp->employee_id }}) </option>
                                    @endforeach
                        </select>
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('rule') ? ' has-danger' : '' }}">
                     ...
                     value="{{ old('rule') }}" aria-required="true">
                        <option value=" ">Select</option>
                        @foreach($cde as $emp)
                        <option value="{{ $emp->employee_id }}" {{$cde_reporting_to[0]->cde_id == $emp->employee_id  ? 'selected' : ''}}  >{{ $emp->first_name }} {{ $emp->middle_name }} {{ $emp->surname }} ({{ $emp->employee_id }}) </option>
                        @endforeach
                        </select>
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    ...
                    <input type="text" value="{{ old('rule',date('d-m-Y', strtotime($cde_reporting_to[0]->reporting_date))) }}" class="form-control datepicker" id="report_date" placeholder="Reporting Date" name="report_date">
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    ...
                    <input type="text" value="{{ old('rule', date('d-m-Y', strtotime($cde_reporting_to[0]->reporting_end_date)) ) }}" class="form-control datepicker" id="report_end_month" placeholder="Reporting Date" name="report_end_month">
