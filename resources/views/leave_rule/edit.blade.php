...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          ...
          <form method="post" action="{{ route('leave_rule.update', $leave_rule[0]->leave_rule_id ) }}" autocomplete="off" class="form-horizontal">
            @csrf
            @method('put')
            ...
            <div class="card ">
              <div class="card-header card-header-rose card-header-icon">
                ...
                <h4 class="card-title">{{ __('Edit Leave') }}</h4>
              </div>
    ...
    <option value="{{$leave_rule[0]->leave_type}}" @if($leave->leave_rule_id == $leave_rule[0]->leave_type) {{ 'selected' }} @endif>{{$leave->leave_type}}</option>
    ...
    <option value="{{$leave_rule[0]->leave_type}}" @if ($leave->leave_rule_id == $leave_rule[0]->leave_type) {{ 'selected' }} @endif>{{$leave->leave_type}}</option>
              ...
              @endforeach 
                  ...
                  <div class="col-sm-7">
                    ...
                    <input class="form-control{{ $errors->has('total_days') ? ' is-invalid' : '' }}" name="total_days" id="input-name" type="number" placeholder="{{ __('Total Days') }}" value="{{ old('total_days',$leave_rule[0]->total_days) }}"  aria-required="true"/>
                ...
                <!-- <div class="row">
                  ...
                  <div class="col-sm-7">
                    ...
                    <input class="form-control datepicker {{ $errors->has('year') ? ' is-invalid' : '' }}" name="year" id="input-year" type="text" placeholder="{{ __('Year') }}" value="{{ old('year',$leave_rule[0]->year) }}"  aria-required="true"/>
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('requirements') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('requirements') ? ' is-invalid' : '' }}" name="requirements" id="input-requirements" type="text" placeholder="{{ __('Requiremnts') }}" value="{{ old('requirements',$leave_rule[0]->requirements) }}" />
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('remarks') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('remarks') ? ' is-invalid' : '' }}" name="remarks" id="input-remarks" type="text" placeholder="{{ __('Remarks') }}" value="{{ old('remarks',$leave_rule[0]->remarks) }}" />
