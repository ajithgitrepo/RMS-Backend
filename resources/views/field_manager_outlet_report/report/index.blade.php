...
@extends('layouts.app', ['activePage' => 'field_outlet_report', 'menuParent' => 'Field_Manager_Report', 'titlePage' => __('Outlet Report')])
            ...
            <form method="" action="" autocomplete="off" class="form-horizontal">
                   ...
                   <select class="form-control js-select2" data-style="select-with-transition"  title="Select Outlet" data-size="7" name="outlet_id" id="input_outlet_id" 
                     value="{{ old('outlet_id') }}" aria-required="true" required >
                      ...
                      @foreach ($outlets as $out)
                        ...
                        <option value="{{ $out->outlet_id}}"> {{ $out->store[0]->store_code }} - {{ $out->store[0]->store_name  }} - {{ $out->store[0]->address }} - {{ $out->outlet_city }} </option>
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    ...
                    <select class="form-control js-select2" data-style="select-with-transition"  title="Sku / Item / Product " data-size="7" name="activity[]" value="{{ old('activity') }}" aria-required="true" id="activity" required>
                 ...
                 <div class="row">
                  ...
                  <div class="col-sm-7">
                   <select class="form-control js-select2" data-style="select-with-transition"  title="Select Merchandiser" data-size="7" name="merchandiser" id="input_merchandiser" 
                     value="{{ old('merchandiser') }}" aria-required="true" required >
                      <option value="" selected="" >-- All Merchandiser --</option>

                    @foreach ($merchandisers as $merchant)

                        <option value="{{ $merchant->employee_id}}"> {{ $merchant->first_name }} {{ $merchant->surname  }} - {{ $merchant->employee_id }} </option>
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    ...
                    <input class="form-control datepicker {{ $errors->has('from_date') ? ' is-invalid' : '' }}" name="from_date" id="input_from_date" type="text" placeholder="{{ __('') }}" value="{{ old('from_date') }} "   aria-required="true" required>
                 ...
                 <div class="row">
                  ...
                  <div class="col-sm-7">
                    ...
                    <input class="form-control datepicker {{ $errors->has('to_date') ? ' is-invalid' : '' }}" name="to_date" id="input_to_date" type="text" placeholder="{{ __('') }}" value="{{ old('to_date') }} "   aria-required="true" required>
                ...
                <button type="submit" id="get_report" class="btn btn-rose mx-auto">{{ __('Get Report') }}</button>
              </div>
...
@push('js')
  ...
  <script>
    ...
    $("#get_report").click(function(e) { 
        ...
        var emp_id = $("#input_merchandiser").val();
        ...
        $.ajax({
          url: '/get_field_report',
          type: 'POST',
          data: {outlet_id : outlet_id, activity : activity, emp_id : emp_id, from_date : from_date, to_date : to_date, '_token': csrf},
          dataType: 'json',
          ...
          success: function( data ) {
                ...
                if(activity =="Visibility") 
                {
                        ...
                        $.each(data, function (key, val) {
                            ...
                            if(data[key].is_available == 1)
                            {
                                html += '<td hidden> visibility_image/'+data[key].image_url+' </td>';
                            }
